...
export async function readCsvFile(filePath: string, fieldMappings: CsvColumnMapping): Promise<CsvRow[]> {
        ...
        fs.createReadStream(filePath)
            ...
            .on('data', (data) => {
                ...
                for (const [csvHeader, fieldName] of Object.entries(fieldMappings)) {
                    normalized[fieldName] = data[csvHeader] || data[csvHeader.toLowerCase()] || data[csvHeader.toUpperCase()];
                }
            ...
            .on('error', (error) => {
                reject(new CsvValidationError(`Failed to read CSV file: ${error.message}`));
            });
    });
}

export function validateRequiredFields(row: CsvRow, requiredFields: string[], rowIndex: number): void {
    for (const field of requiredFields) {
        if (!row[field]) {
            throw new CsvValidationError(`Missing required field '${field}' at row ${rowIndex + 1}`, rowIndex + 1);
        }
    }
}
